...
r_in = /\/dev\/grid\/node-x(\d+)-y(\d+)\s+(\d+)T\s+(\d+)T\s+(\d+)T\s+(\d+)%/
parse = ( input )->
	out = for l in input.split '\n'
		r = r_in.exec l
		if not r then continue
		r[1..6].map (a)->+a

count_viable_pairs = ( df )->
	num = 0
	for a, i in df
		assert a[2], a
...
do ->
	try
		_log.darkGray df = parse input
		_log.yellow count_viable_pairs df
		g = create_grid df
		s = g[0][g[0].length-1]
		d = g[0][0]
		for r, y in g
			_print (for a, x in r
				...
				switch
					when a[3] is 0
						s.green.inverse
					when not x and not y
						s.cyan.inverse
					when not y and x is g[0].length-1
						s.yellow.inverse
					when a[3] > 90
						s.red.inverse
					else
		...
		_log.red "source data #{s[3]}, free #{s[4]}"
		_log.green "dest data #{d[3]}, free #{d[4]}"
		_log.cyan "now, look at the map and find the optimal way. It is #{'205'.red} steps long. =) #{'50'.red} to move free space #{'(0:90)'.green} to source node at x32:y0, then #{'31x5'.red} steps to move data to destination at x0:y0"
	catch e
		_log.red e
	return
