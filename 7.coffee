	...
	for c, i in ip
		switch c
			...
			when ']'
				...
				if ip[i] is ip[i+3] and ip[i+1] is ip[i+2] and ip[i] isnt ip[i+1] and '[' not in ip[i..i+3] and ']' not in ip[i..i+3]
					if inside
						return no
					else
						tls = yes
...
# _log has_tls 'abba[asdf]zxcv'
# _log has_tls 'abza[asdf]zxcv'
# _log has_tls 'abba[adda]zxcv'

count = ( f, ips )->
	num = 0
	for ip in ips when f ip
		num++
	num
...
has_ssl = ( ip )->
	...
	for c, i in ip
		switch c
			...
			when ']'
				...
				if c is ip[i+2] and c isnt ip[i+1] and ip[i+1] not in '[]'
					if inside
						babs[ip[i..i+2]]=yes
					else
						abas[ip[i..i+2]]=yes
	for aba of abas
		for bab of babs
			if aba[0] is bab[1] and aba[1] is bab[0]
				return yes
	no
	

_log 'tls', count has_tls, ips
_log 'ssl', count has_ssl, ips
