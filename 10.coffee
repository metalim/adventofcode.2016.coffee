...
create_bots = ( rules )->
	...
	for rule in rules
		switch
			when res = /value (\d+) goes to bot (\d+)/.exec rule
				add_chip_to +res[1], bots[res[2]] ?= []
			when res = /bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/.exec rule
				bot = bots[res[1]] ?= []
				switch res[2]
					when 'bot'
						bot[2] = +res[3]
...
do_step = ( bots, out )->
	active = 0
	for bot, i in bots when bot[0]? and bot[1]?
		active++
		if bot[2]?
			add_chip_to bot[0], bots[bot[2]]
		if bot[3]?
			add_chip_to bot[1], bots[bot[3]]
		if out?
			if bot[4]?
				out[bot[4]] = bot[0]
			if bot[5]?
...
find_bot = ( rules, a, b )->
	bots = create_bots rules
	step = 0
	while yes
		active = do_step bots

		for bot, i in bots when a is bot[0] and b is bot[1]
			return i
...
mul_out_012 = ( rules )->
	...
	out = []
	while active
		active = do_step bots, out
		step++
	_log out[0..2]
	out[0] * out[1] * out[2]

_log find_bot input, 17, 61
_log mul_out_012 input
