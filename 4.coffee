...
get_checksum = ( str )->
	arr = for c, freq of get_freqs str
		[c,freq]
	arr.sort ( a,b )->
		b[1]-a[1] or a[0].charCodeAt()-b[0].charCodeAt()
	#_log arr[...5]
	arr[...5].reduce (a,v)->
		a+v[0]
	, ''
...
is_real = ( room )->
	res = /(.*)-(\d+)\[(\w*)\]/.exec room
	name = res[1].replace /-/g,''
	code = +res[2]
	checksum = res[3]
	#_log name, code, checksum, get_freqs name
	if checksum is get_checksum name
		return code
	0


sum_real_room_codes = ( rooms )->
	sum = 0
	for room in rooms
		sum += is_real room
	sum
...
decrypt = ( encrypted, code )->
	name = for c in encrypted
		switch c
			when '-' then ' '
			else
				String.fromCharCode (c.charCodeAt()-A + code)%%ZA + A
	name.join ''

#_log decrypt 'qzmt-zixmtkozy-ivhz', 343 #'very encrypted name'

decrypt_names = ( rooms )->
	names = for room in rooms when is_real room
		res = /(.*)-(\d+)\[\w*\]/.exec room
		"#{decrypt res[1], +res[2]} : #{res[2]}"
	names

_log sum_real_room_codes rooms
names = decrypt_names rooms
for n in names when ~n.indexOf 'north'
	_log n
